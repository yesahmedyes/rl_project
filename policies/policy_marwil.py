...
class Policy_MARWIL:
    def __init__(
        ...
        if not ckpt.exists():
            raise FileNotFoundError(
                ...
                "Provide a valid path from train_marwil.py outputs."
            )
    ...
    def _normalize_checkpoint_path(path_str: str) -> Path:
        """Handle raw paths and Ray repr strings like TrainingResult(checkpoint=...)."""
        ckpt = Path(path_str)
        ...
        if match:
            ...
            latest = Policy_MARWIL._find_latest_checkpoint(candidate)
            if latest:
                return latest

        if ckpt.is_dir():
            latest = Policy_MARWIL._find_latest_checkpoint(ckpt)
            if latest:
                return latest
