...
class Policy_Heuristic:
    ...
    def evaluate_heuristic(self, state, action):
        ...
        def convert_to_opponent_position(position):
            temp_goti = Goti(position=position)

            return temp_goti.convert_into_opponent_position()
        ...
        if current_pos == STARTING and dice_value == 6:
            ...
            spawn_from_opp_view = convert_to_opponent_position(0)
            if spawn_from_opp_view not in opp_gotis:
